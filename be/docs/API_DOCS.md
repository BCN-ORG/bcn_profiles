...
- [Users](#-users)
- [Upload avatar MinIO](#upload-avatar-minio)
...
Backend chỉ nhận avatar thuộc folder của user hiện tại và URL khớp object key. Đổi avatar sẽ yêu cầu xóa object cũ. `DELETE /users/me/avatar` không có body, trả user cập nhật ở `data.users`; không dùng endpoint này để xóa object mới chưa gắn. Đọc lại profile bằng `GET /users/me/profile`.
