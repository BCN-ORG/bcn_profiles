...
export class AuthService {
  ...
  constructor(
    ...
    @Optional() private readonly identitySessions?: IdentitySessionService,
  ...
  async validateUser(email: string, password: string): Promise<any> {
    ...
    if (
      ...
      await this.writeAudit(user?.id ?? null, 'LOGIN_FAILED', { email });
      return null;
    }
  ...
  async generateTokensAfterTwoFactorVerification(userId: string): Promise<{
    ...
    const [tokens, ssoSessionId] = await Promise.all([
      this.issueTokenPair(user),
      this.identitySessions?.createSso(
        user.id,
        user.twoFactorEnabled ? 'AAL2' : 'AAL1',
      ),
    ]);
    await this.writeAudit(user.id, 'LOGIN_SUCCESS');

    return {
      ...tokens,
      ...(ssoSessionId ? { sso_session_id: ssoSessionId } : {}),
      user: {
        id: user.id,
        email: user.email,
  ...
  private async writeAudit(
    ...
    await this.prisma.authAuditLog?.create({
      data: { id: randomUUID(), userId, eventType, metadata },
    });
  }
}
