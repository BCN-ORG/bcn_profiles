...
export function createBcnIdentityArtifact(
  ...
  return {
    ...
    tick: (_dt, elapsed) => {
      corePivot.position.y = Math.sin(elapsed * 1.05) * 0.04;
      ...
      softRingMat.opacity = 0.14 + pulse * 0.12;
...
export function createArtifactSceneLights(scene: THREE.Scene) {
  ...
  scene.add(fill);
  const rim = new THREE.PointLight(0x3cb887, 12, 10, 2);
